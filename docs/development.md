...
D1 и KV сейчас не подключены. PixiJS остаётся предварительным кандидатом на игровой renderer;
текущая стартовая страница его не требует. В API пока только `GET /api/health`.
...
pnpm test       # HTTP-тесты Hono через Vitest
...
Frontend пока статический; отдельные unit-тесты декоративного текста не добавлены.
