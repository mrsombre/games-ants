...
`Ant` — объединение по `role`: у всех ролей есть маршрут, признак свободной прогулки
и таймер до следующего выбора. Задание стройки или переноса яиц и работа есть только
у рабочего, груз и таймер вылазки — только у разведчика, фаза поверхности — только у воина.
